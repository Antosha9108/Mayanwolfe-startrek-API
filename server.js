...
// set up get method for home page/ root route
app.get('/', (request, response) => {
    response.sendFile(__dirname + '/index.html')
})

// set up get method for json/api
// '/api/:alienName' is a route for our api values. We could call it anything (unicorn, robot)
// we declared aliensName to have it as a variable so it is easier to pass over to params and etc.
//then we check if aliensName is in the aliens array and return json if it is. return humans if it is not(default fallback value)
...
// we are setting up for server to listen on PORT/8001 or for PORT if we put it on heroku
